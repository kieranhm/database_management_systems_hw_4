...
public:
    ...
    void write_into_data_file(ostream &out) const {
        ...
        for (int i = slot_directory.size() - 1; i > 0; i--) {
            char* slot_string;
            sprintf(slot_string, "%d, %d", slot_directory[i].first, slot_directory[i].second);
            strcat(page_data, slot_string);
        }
        //  - Write overflowPointerIndex into page_data buffer.
        char* overflowPointerString;
        sprintf(overflowPointerString, "%d", overflowPointerIndex);
        strcat(page_data, overflowPointerString);
